...
%
% This code follows the normalised direct linear transformation(DLT) algorithm 
% given by Hartley and Zisserman "Multiple View Geometry in Computer Vision" p92.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% H=A*[r1,r2,t];
function H = homography2d(varargin)
    ...
    % Attempt to normalise each set of points so that the origin is at centroid and mean distance from origin is sqrt(2).
    [x1, T1] = normalize2dpts(x1); % call normalise2dpts.m
    [x2, T2] = normalize2dpts(x2);
    
    % Note that it may have not been possible to normalise the points if one was at infinity so the following does not
   ...
   'Algorithm','Levenberg-Marquardt');
    [x,resnorm,residual,exitflag,output]  = lsqnonlin( @MLE1, reshape(H,1,9) , [],[],options,m, M);
    H=reshape(x,3,3);
    H=H/H(3,3);   
