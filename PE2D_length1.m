%% -------------------- PE2D_length1.m-----------------------
% Usage:Calculate PE2D for sub-image, with a window sliding along short
%       edge, to determine the world coorinate      
%

function [phase1,phase2]=PE2D_length1(M_in,data,row_num,row_start,row_end,col_start,col_end)
        i=0;
    for k=row_start:M_in-row_end+1       
        data_im=data(k:k+row_num-1,col_start:col_end);
        i=i+1;
        [N,M]=size(data_im);
        ...
        L_square = 10;%unit:pixel
        
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       %calculate the first sub-image
        if(i==1)
            [phase1,phase2,u1,v1,u2,v2] = PE2D(data_im); % call PE2D.m
            phase1(i)=phase1;
            phase2(i)=phase2;
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % search the maximum points in spectrum
            SearchSquare_top_1(i) = max((row1(i-1)-L_square),1);%one direction
            ...
            SearchSquare_top_2(i) = max((row2(i-1)-L_square),1);%the other direction
